...
if ($resource === 'metiers' && $method === 'GET') {
    $id !== null ? MetierController::show($id) : MetierController::index();
    exit;
}

if ($resource === 'mentions' && $method === 'GET') {
    $id !== null ? MentionController::show($id) : MentionController::index();
    exit;
}

if ($resource === 'domaines' && $method === 'GET') {
    $id !== null ? DomaineController::show($id) : DomaineController::index();
    exit;
}

if ($resource === 'series' && $method === 'GET') {
    $id !== null ? SerieController::show($id) : SerieController::index();
    exit;
}

if ($resource === 'parcours' && $method === 'GET') {
    $id !== null ? ParcoursController::show($id) : ParcoursController::index();
    exit;
}

if ($resource === 'etablissements' && $method === 'GET') {
    $id !== null ? EtablissementController::show($id) : EtablissementController::index();
    exit;
}
...
if ($resource === 'profile') {
    ...
    match ($method) {
        ...
        default => Response::json(['message' => 'Methode non autorisee'], 405),
    };
    exit;
}
...
if ($resource === 'users') {
    ...
    match (true) {
        ...
        $method === 'POST' => AuthController::register($currentUser),
